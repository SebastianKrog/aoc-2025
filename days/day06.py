...
def part1(data: Any) -> Any:
    """Solve part 1."""
    data = [re.split(r"\s+", l.strip()) for l in data]
    data = zip(*[data[-1]] + [[int(a) for a in l] for l in data[:-1]])

    zum = 0
    for terms in data:
        if terms[0] == "*": zum += prod(terms[1:])
        else: zum += sum(terms[1:])
    return zum


def part2(data: Any) -> Any:
    """Solve part 2."""
    data = zip(*[reversed(l) for l in data])

    zum = 0
    stack = []
    for col in data:
        if all(x == " " for x in col): continue
        nums, symbol = col[:-1], col[-1]
        stack.append(int("".join(nums)))
        if symbol == "+": zum += sum(stack)
        if symbol == "*": zum += prod(stack)
        if symbol != " ": stack = []
